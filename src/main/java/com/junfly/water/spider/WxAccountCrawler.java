...
//    Proxys proxys=new Proxys();
    ...
    public WxAccountCrawler(String crawlPath, String historyKeysPath) throws Exception {
        ...
        historyKeysWriter = new BufferedWriter(
                new OutputStreamWriter(new FileOutputStream(historyKeysPath, true), "utf-8"));
//        proxys.add("218.15.25.153", 808);
//        proxys.add("118.114.77.47", 8080);
//        proxys.add("123.168.124.139", 8118);
//        proxys.add("218.15.25.153", 808);
    }
        ...
        if (page.matchType("account_search")) {
            ...
            for (int i = 0; i < articleJSONArray.length(); i++) {
                ...
                String articleUrl = "http://mp.weixin.qq.com"
                        + articleJSON.getString("content_url").replace("&amp;", "&");
            ...
            try {
                ...
                articleJSON.append("account", account).append("title", title).append("date", date).append("content",
                        content);
                System.out.println(articleJSON);
                PybbsTopic pybbsTopic = new PybbsTopic();
				pybbsTopic.setContent(content);
				pybbsTopic.setTitle(title);
...
//                pybbsTopicService.save(pybbsTopic);
    ...
    public void addAccount(String account) throws UnsupportedEncodingException {
        String seedUrl = "http://weixin.sogou.com/weixin?type=1&" + "s_from=input&ie=utf8&query="
